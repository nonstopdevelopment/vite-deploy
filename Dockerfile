...
# We explicitly add --host 0.0.0.0 to npm run dev if it's Vite
CMD ["sh", "-c", "echo 'Starting application...' && (npm run dev -- --host 0.0.0.0 || npm start || node index.js || (echo 'Start failed' && exit 1))"]
